...
export default {
  ...
  computed: {
    UpdataTotal() {
        return (
          "当前文件上传总数为：" +
          this.$store.state.upadaFileNumberSum +
          "个文件,已经上传" +
  ...
  methods: {
    SuLvZhuangHuang(i) {
      if (i < 1024) {
        this.$store.state.SuLvZhuangHuang_Data_vuex=i.toFixed(2) + " B/s";
        return i.toFixed(2) + " B/s";
      } else if (i < 1024 * 1024) {
        this.$store.state.SuLvZhuangHuang_Data_vuex=(i / 1024).toFixed(2) + " KB/s";
        return (i / 1024).toFixed(2) + " KB/s";
      } else if (i < 1024 * 1024 * 1024) {
        this.$store.state.SuLvZhuangHuang_Data_vuex=(i / (1024 * 1024)).toFixed(2) + " MB/s";
        return (i / (1024 * 1024)).toFixed(2) + " MB/s";
      } else {
        this.$store.state.SuLvZhuangHuang_Data_vuex=(i / (1024 * 1024 * 1024)).toFixed(2) + " GB/s";
