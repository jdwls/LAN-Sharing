...
function getStyleDir(compName) {
  ...
  if (!styleDir)
    styleDir = _chunkMU27IQTBcjs.kebabCase.call(void 0, compName);
  return styleDir;
}
...
function getComponentStyleDir(importName, importStyle) {
  if (["ConfigProvider", "Icon"].includes(importName))
    return void 0;
  let componentDir = _chunkMU27IQTBcjs.kebabCase.call(void 0, importName);
  ...
  return {
    ...
    resolve: (name) => {
      if (canResolveIcons(options.resolveIcons)) {
        const iconPrefix = _chunkMU27IQTBcjs.pascalCase.call(void 0, getResolveIconPrefix(options.resolveIcons));
      ...
      if (name.match(/^A[A-Z]/) && !_chunkMU27IQTBcjs.isExclude.call(void 0, name, options.exclude)) {
...
function componentsResolver(name, { ssr }) {
  if (!name.match(/^D[A-Z]/))
    return;
  const resolveId = _chunkMU27IQTBcjs.kebabCase.call(void 0, name = name.slice(1));
  return {
    name,
...
function directivesResolver(name, { ssr }) {
  const resolveId = _chunkMU27IQTBcjs.kebabCase.call(void 0, name);
  return {
    name: `${name}Directive`,
    sideEffects: getSideEffects2(resolveId, "style.css"),
...
function resolveComponent(name, options) {
  ...
  const partialName = _chunkMU27IQTBcjs.kebabCase.call(void 0, name.slice(2));
...
function ElementPlusResolver(options = {}) {
  ...
  async function resolveOptions() {
    ...
    optionsResolved = {
      ssr: false,
      version: await _chunkMU27IQTBcjs.getPkgVersion.call(void 0, "element-plus", "2.2.2"),
      importStyle: "css",
      directives: true,
      exclude: void 0,
  ...
  return {
    ...
    resolve: (name) => {
      ...
      if (/^El[A-Z]/.test(name)) {
        ...
        const partialName = _chunkMU27IQTBcjs.kebabCase.call(void 0, compName);
...
function IduxResolver(options = {}) {
  return {
    ...
    resolve: async (name) => {
      ...
      if (!packageName)
        return;
      const resolvedVersion = await _chunkMU27IQTBcjs.getPkgVersion.call(void 0, `${scope}/${packageName}`, "2.0.0");
      let dirname = specialComponents[name];
      if (!dirname) {
        const nameIndex = packageName === "pro" ? 2 : 1;
        dirname = _chunkMU27IQTBcjs.kebabCase.call(void 0, name).split("-")[nameIndex];
      }
...
function resolveComponent2(importName, options) {
  let name;
  if (_chunkMU27IQTBcjs.isExclude.call(void 0, importName, options.exclude))
    return void 0;
...
function TDesignResolver(options = {}) {
  ...
  return {
    ...
    resolve: (name) => {
      ...
      if (_chunkMU27IQTBcjs.isExclude.call(void 0, name, exclude))
        return;
...
var moduleType = _chunkMU27IQTBcjs.isSSR ? "lib" : "es";
function getSideEffects7(dirName, options) {
  const { importStyle = true } = options;
  if (!importStyle || _chunkMU27IQTBcjs.isSSR)
    return;
  if (importStyle === "less")
    ...
    resolve: (name) => {
      if (name.startsWith("Van")) {
        ...
        return {
          ...
          sideEffects: getSideEffects7(_chunkMU27IQTBcjs.kebabCase.call(void 0, partialName), options)
        };
      }
    }
  };
...
function getResolved(name, options) {
  ...
  if (importStyle || importCss) {
    if (importStyle === "less" || importLess)
      sideEffects.push(`${path}/es/${_chunkMU27IQTBcjs.kebabCase.call(void 0, name)}/style/less`);
    else
      sideEffects.push(`${path}/es/${_chunkMU27IQTBcjs.kebabCase.call(void 0, name)}/style/index${styleExtname}`);
  }
  return {
    from: path,
...
var formatters = {
  "kebab-case": _chunkMU27IQTBcjs.kebabCase,
  "camelCase": _chunkMU27IQTBcjs.camelCase,
  "PascalCase": _chunkMU27IQTBcjs.pascalCase
};
...
function getCompDir(compName) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (compName.match(matcher.pattern)) {
      compPath = `${matcher.compDir}/${_chunkMU27IQTBcjs.kebabCase.call(void 0, compName)}.vue`;
      break;
    }
  }
  if (!compPath)
    compPath = _chunkMU27IQTBcjs.kebabCase.call(void 0, compName);
  return compPath;
}
